...
if(require.main == module) {
 program
  ...
  .option('-u, --url <url_path>', 'URL Path to checked')
  .parse(process.argv);
 ...
 else if(program.url != null && program.url != '')
 { 
   console.log("checking URL: %s", program.url);
   //checkJson = checkUrlPath(program.url, program.checks);
   restler.get(program.url).on('complete', function(result){
     if(result instanceof Error){
      console.log("Problem getting the specified URL: %s", result);
      process.exit(1);
     }else{
      $=cheerio.load(result);
 ...
 {
   console.log("no file nor url parameter found. check --help for usage");
 }
} else {
